...
public class MyLifeInStardew {
    ...
    private String getDetails(TaskInterface.TaskType taskType) {
        ...
        if (taskType == Task.TaskType.FISHING) {
            String[] fishingDetails = { "in the Ocean", "in the River", "in the Pond", "in the Lake",
                    "in the Hatcheries", };
            return fishingDetails[detailIndex];
        }
        if (taskType == Task.TaskType.FARM_MAINTENANCE) {
            String[] maintenanceDetails = { "of Harvesting Wheat", "of Clearing Rocks", "of Watering Plants",
                    "of Wood Chopping", "of Beautification", };
        ...
        if (taskType == Task.TaskType.SOCIALIZING) {
            String[] socialDetails = { "with Mayor Lewis", "with Store Owner Pierre", "with Carpenter Robin",
                    "with Blacksmith Clint", "with the Wizard", };
